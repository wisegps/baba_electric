...
public class FragmentHome extends Fragment {
	...
	private void getCards() {
		...
		for (int i = 0; i < sharedCards.length; i++) {
			...
			}else if (cardName.equals(Const.TAG_ELECTIRC)) {
				Log.i("fragment", "设置净化器卡片布局");
				removeFragment(Const.TAG_ELECTIRC);
				transaction = fragmentManager.beginTransaction();
				FragmentElectricCarInfo fragmentElectric = new FragmentElectricCarInfo();
				fragmentElectric.setOnCardMenuListener(onCardMenuListener);
				transaction
						.add(R.id.ll_cards, fragmentElectric, Const.TAG_ELECTIRC);
				transaction.commit();
				cards.put(Const.TAG_ELECTIRC, fragmentElectric);
			}
		}
	}
...
//		}
		...
		FragmentElectricCarInfo fragmentElectricCarInfo = (FragmentElectricCarInfo) cards.get(Const.TAG_ELECTIRC);
